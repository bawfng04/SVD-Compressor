...
# python SVDCompresser_simplified.py

# Ảnh gốc có kích thước m x n
# Phân tích ảnh bằng SVD sẽ cho ra ma trận U (m x m), ma trận S (m x n) và ma trận Vt (n x n)
# Trong đó:
# - U chứa các vector riêng của ảnh gốc (ma trận suy biến trái) -> Đặc trưng theo hàng
# - S là ma trận đường chéo chứa các giá trị suy biến, thứ tự giảm dần, biểu thị cho mức độ quan trọng của các thành phần
# - Vt chứa các vector riêng của ảnh gốc (ma trận suy biến phải) -> Đặc trưng theo cột
# Chỉ cần giữ lại k thành phần đầu tiên của U, S và Vt để nén ảnh (k thành phần quan trọng nhất)

# -> từ m x n, phân tích thành m x k, k x k, k x n -> (m + n + 1) * k số lượng tham số cần lưu trữ

def calculate_psnr(original: np.ndarray, compressed: np.ndarray) -> float:
    # tính PSNR (Peak Signal-to-Noise Ratio), PSNR càng cao, chất lượng ảnh nén càng tốt
    # PSNR là sự khác biệt về pixel giữa ảnh gốc và ảnh nén, giá trị vô hạn -> Ảnh nén hoàn toàn giống ảnh gốc
    # mse là trung bình bình phương sai số giữa ảnh gốc và ảnh nén
    mse = np.mean((original - compressed) ** 2)
    if mse == 0: # nếu không có sai số, PSNR là vô hạn, nghĩa là ảnh nén hoàn toàn giống ảnh gốc
        return float("inf")
    max_pixel = 255.0
    psnr = 20 * np.log10(max_pixel / np.sqrt(mse)) #nếu có sai số, tính PSNR theo công thức
    return psnr # đơn vị: dB (decibel)


# tính SSIM (Structural Similarity Index)
# SSIM đo lường độ tương đồng cấu trúc giữa hai ảnh
def calculate_ssim_simple(original: np.ndarray, compressed: np.ndarray) -> float:
    mu1 = np.mean(original) # trung bình của ảnh gốc
    mu2 = np.mean(compressed) # trung bình của ảnh nén
    sigma1_sq = np.var(original) # phương sai của ảnh gốc
    sigma2_sq = np.var(compressed) # phương sai của ảnh nén
    sigma12 = np.mean((original - mu1) * (compressed - mu2)) # hiệp phương sai giữa ảnh gốc và ảnh nén

    c1 = (0.01 * 255) ** 2  # hằng số để tránh chia cho 0, c1 và c2 là các hằng số nhỏ để ổn định tính toán
    c2 = (
        0.03 * 255
    ) ** 2  # Mục đích là để tránh việc chia cho 0 khi mẫu số trong công thức SSIM quá nhỏ (ví dụ, trong các vùng ảnh có độ tương phản thấp).

    # SSIM: Chỉ số tương đồng cấu trúc, bao gồm độ sáng, độ tương phản và cấu trúc của ảnh
    # Có giá trị từ -1 đến 1, với 1 là hoàn toàn tương đồng
...
def save_compressed_images(
    ...
    # recon_array là mảng numpy của ảnh nén
    # k là số thành phần đã sử dụng để nén
    # image_type là loại ảnh (gray hoặc color)
    # output_dir là thư mục lưu ảnh nén, mặc định là "output"
...
# tính toán giá trị k tối ưu dựa trên năng lượng tích lũy
# để đạt được ít nhất 95% năng lượng
# Một giá trị suy biến σ lớn -> σ² càng lớn -> thành phần đó chứa nhiều năng lượng.
# Tổng năng lượng của ảnh là tổng của tất cả các σ².
def find_optimal_k(S: np.ndarray, energy_threshold: float = 0.95) -> int:
    cumulative_energy = np.cumsum(S**2) / np.sum(S**2) # tính năng lượng tích lũy
    # S: [10, 8, 3, 1] -> S**2: [100, 64, 9, 1]
    # -> np.sum(S**2): 100 + 64 + 9 + 1 = 174
    # np.cumsum(S**2) = [100, 100+64, 100+64+9, 100+64+9+1] = [100, 164, 173, 174].
    # cumulative_energy = [100/174, 164/174, 173/174, 174/174] = [0.57, 0.94, 0.99, 1.0].

    # năng lượng tích luỹ theo công thức: # cumulative_energy[i] = sum(S[:i+1]^2) / sum(S^2)
    # với S là mảng các giá trị suy biến, # cumulative_energy[i] là năng lượng tích lũy đến thành phần thứ i
    # tìm k sao cho năng lượng tích lũy >= energy_threshold

    # cumulative_energy: [0.57, 0.94, 0.99, 1.0] -> [False, False, True, True]
    # -> np.argmax trả về chỉ số đầu tiên mà cumulative_energy >= energy_threshol
    # -> +1 để chuyển từ chỉ số (bắt đầu từ 0) sang giá trị k (bắt đầu từ 1)
    # -> k = 2 (index) + 1 = 3
    optimal_k = np.argmax(cumulative_energy >= energy_threshold) + 1 # tìm k sao cho năng lượng tích lũy >= 95%
    return optimal_k
...
# nén ảnh từ URL và hiển thị kết quả
#
#
#
def compress_and_show_images(
    ...
    try:
        ...
        original_img_array = np.array(img, dtype=np.float64) # chuyển đổi ảnh thành mảng numpy
    ...
    gray_img_array = np.mean(original_img_array, axis=2) # chuyển đổi ảnh màu sang ảnh xám
    U, S, Vt = np.linalg.svd(gray_img_array, full_matrices=False)

    optimal_k = find_optimal_k(S, 0.95) # tìm giá trị k tối ưu để đạt ít nhất 95% năng lượng
    ...
    for i, k in enumerate(k_values):
        ...
        # lấy 3 ma trận U, S, Vt với k thành phần đầu tiên
        ...
        if save_results:
            save_compressed_images(reconstructed_array, k, "gray") # lưu ảnh nén vào thư mục output
    ...
    R, G, B = (
        # tách các kênh màu R, G, B từ ảnh gốc,
...
def compress_from_array(original_img_array: np.ndarray, k_values: List[int]):
    ...
    for i, k in enumerate(k_values):
        ...
        reconstructed_array = U[:, :k] @ np.diag(S[:k]) @ Vt[:k, :]
        # reconstructed_array là ảnh nén từ k thành phần đầu tiên của U, S, Vt

        compressed_size = k * (m + n + 1)
        compression_ratio = compressed_size / original_size

        mse = np.mean((gray_img_array - reconstructed_array) ** 2) # mse: biểu thị sai số trung bình bình phương giữa ảnh gốc và ảnh nén
        psnr = calculate_psnr(gray_img_array, reconstructed_array) # tính PSNR (Peak Signal-to-Noise Ratio), PSNR càng cao, chất lượng ảnh nén càng tốt
        ssim = calculate_ssim_simple(gray_img_array, reconstructed_array) # tính SSIM (Structural Similarity Index), SSIM càng cao, chất lượng ảnh nén càng tốt
